  ...
  return (
    <>
      <div className="header">
        <div>
          <div className="header-title">💰 BhaarathaFinance</div>
          <div className="header-sub">Trusted Loan Book Dashboard</div>
        </div>
        <button className="sync-btn" onClick={onSync}>{loading ? "..." : "⟳ Sync"}</button>
      </div>
      <div className="page-pad">
      ...
      {investors.length > 0 && (() => {
        ...
        // Interest collected so far = total payments received minus total principal disbursed
        // Principal disbursed is the actual cash given out; anything collected above that is interest
        const interestCollectedSoFar = Math.max(0, totalCollected - totalDisbursed);
        return (
          <>
            <div className="sec-title">Partner Breakdown</div>
            {interestCollectedSoFar > 0 && (
              <div style={{ margin: "0 16px 10px", background: "#f0faf4", border: "1px solid #b7e4c7", borderRadius: 12, padding: "10px 14px" }}>
                <div style={{ fontSize: 11, color: "#1a7a3f", fontWeight: 700, marginBottom: 2 }}>Total Interest Collected So Far</div>
                <div style={{ fontSize: 18, fontWeight: 700, color: "#1a7a3f" }}>{fmt(interestCollectedSoFar)}</div>
                <div style={{ fontSize: 11, color: "#888", marginTop: 2 }}>Distribute this as profit among partners</div>
              </div>
            )}
            {investors.map((inv, i) => {
              ...
              return (
                <div key={i} style={{ margin: "0 16px 8px", background: "#f9f9f9", border: "1px solid #ebebeb", borderRadius: 12, padding: "12px 14px" }}>
                  ...
                  <div style={{ marginTop: 10, padding: "8px 10px", background: myInterest > 0 ? "#f0faf4" : "#f5f5f5", borderRadius: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div style={{ fontSize: 11, color: myInterest > 0 ? "#1a7a3f" : "#999" }}>🏆 Interest Earned (Share)</div>
